import {
  ICurrencyDataSource,
  IFreighDataSource,
  IHistoryLogDataSource,
  IPkhoaDataSource,
  IPksCurahDataSource,
  IPoPksDataSource,
  IPurchasingDataSource,
  IPurchasingDetailDataSource,
  IPurchasingFreightCostDataSource,
  ISetupsDataSource,
  IStockpileDataSource,
  IUsersDataSource,
  IVendorKontrakDataSource,
} from '../../../data'
import {
  CurrencyEntity,
  EntityUser,
...
export class PurchasingRepository implements IPurchasingRepo {
  constructor(
    private readonly userDataSource: IUsersDataSource,
    private readonly pksCurahDataSource: IPksCurahDataSource,
    private readonly freightDataSource: IFreighDataSource,
    private readonly historyLogDataSource: IHistoryLogDataSource,
    private readonly currencyDataSource: ICurrencyDataSource,
    private readonly stockpileDataSource: IStockpileDataSource,
    private readonly pkhoaDataSource: IPkhoaDataSource,
    private readonly purchasingDataSource: IPurchasingDataSource,
    private readonly poPksDataSource: IPoPksDataSource,
    private readonly vendorKontrakDataSource: IVendorKontrakDataSource,
    private readonly setupsDataSource: ISetupsDataSource,
    private readonly purchasingFreightCostDataSource: IPurchasingFreightCostDataSource,
    private readonly purchasingDetailDataSource: IPurchasingDetailDataSource
  ) {}
  ...
  async updateKodeAksesUser(
    deviced_id: string,
    kode_akses: string
  ): Promise<any> {
    const res = await this.userDataSource.updateKodeAksesUserPurchasing(
      deviced_id,
      kode_akses
    )
    return res
  ...
  async pengajuanPksCurah(
    user_id?: number,
    data?: PksCurahEntity | undefined
  ): Promise<any> {
  ...
  async pengajuanFreight(
    user_id?: number,
    data?: FreightEntity | undefined
  ): Promise<any> {
  ...
  async findBankByPksCurahId(
    id?: number[] | undefined
  ): Promise<PksCurahBankEntity[]> {
    const rows = await this.pksCurahDataSource.selectBankByPksCurahId(id)
    return rows
  }

  async findOnePksCurahBank(
    id?: number | undefined
  ): Promise<PksCurahBankEntity | null> {
  ...
  async findBankByFreightId(
    id?: number[] | undefined
  ): Promise<FreightBankEntity[]> {
  ...
  async findAllStockpile(
    conf?: Pick<ParamsEntity, 'limit' | 'offset' | 'user_id'>
  ...
  async findAllPkhoa(
    conf?: Pick<ParamsEntity, 'limit' | 'offset' | 'search' | 'user_id'>
  ): Promise<{ count: number; rows: PkhoaEntity[] }> {
  ...
  async findOnePkhoaDynamic(
    conf?: Pick<ParamsEntity, 'columnKey' | 'columnValue'> | undefined
  ): Promise<PkhoaEntity[]> {
    const rows = await this.pkhoaDataSource.selectOneDynamic(conf)
    return rows
  }

  async getPkhoaExclude(
    stockpile_id: number,
    vendor_id: number,
    req_payment_date: string
  ): Promise<any> {
    const rows = await this.pkhoaDataSource.selectPkhoaExclude(
      stockpile_id,
      vendor_id,
      req_payment_date
    )
  ...
  async pengajuanKontrakPks(
    user_id?: number,
    data?: TypePengajuanKontrakPks
  ): Promise<any> {
    const resPurchasing = await this.purchasingDataSource.insert(data)
    const resPpn = await this.setupsDataSource.selectByNama(
      process.env.PPN_NAME
    )

    let jumlahPpn = Number(1) + Number(resPpn.nilai!)
    let pricePoPks: number =
      data?.ppn == 1 ? Number(data?.price!) / Number(jumlahPpn) : data?.price!
      ...
      if (tmpFreighCostId.length > 0)
        await Promise.all(
          tmpFreighCostId.map(async (val: number, i: number) => {
            ...
            let dataPurchasingFreightCost = new Map<
              string,
              PurchasingFreightCostEntity
            >()
            ...
            const resVendorKontrak = await this.vendorKontrakDataSource.insert(
              dataVendorKontrak.get('data')
            )
            const resPurchasingFreightCost =
              await this.purchasingFreightCostDataSource.insert(
                dataPurchasingFreightCost.get('data')
              )
            ...
            await this.historyLogDataSource.insert(
              dataHistoryLogPurchasingFreightCost
            )
          })
        )
    }
    ...
    const insertPurchasingDetail = await this.purchasingDetailDataSource.insert(
      dataPurchasingDetail.get('data')
    )
  ...
  async findOnePurchasingDynamic(
    conf?: Pick<ParamsEntity, 'columnKey' | 'columnValue'> | undefined
  ): Promise<PurchasingEntity[]> {
    const rows = await this.purchasingDataSource.selectOneDynamic(conf)
    return rows
  }

  async findOnePoPksDynamic(
    conf?: Pick<ParamsEntity, 'columnKey' | 'columnValue'> | undefined
  ): Promise<PoPksEntity[]> {
  ...
  async findOneVendorKontrakDynamic(
    conf?: Pick<ParamsEntity, 'columnKey' | 'columnValue'> | undefined
  ): Promise<VendorKontrakEntity[]> {
    const rows = await this.vendorKontrakDataSource.selectOneDynamic(conf)
    return rows
  }

  async findAllKontrakPks(
    conf?: ParamsEntity
  ): Promise<{ count: number; rows: PurchasingEntity[] }> {
    const count = await this.purchasingDataSource.count()
    const rows = await this.purchasingDataSource.selectAll(conf!)
    return { count: count.count, rows }
  }

  async findOneKontrakPks(
    id?: number | undefined
  ): Promise<PurchasingEntity | null> {
  ...
  async deletePurchasingFreightCost(
    id?: number | undefined,
    user_id?: number | undefined
  ): Promise<any> {
  ...
  async addTerminKontrakPks(
    data?: PurchasingDetailEntity | undefined
  ): Promise<any> {
    ...
    await this.historyLogDataSource.insert(
      logInsertPurchasingDetail.get('data')
    )

    return insert
  }

  async findOneTerminKontrakPks(
    id?: number | undefined
  ): Promise<PurchasingEntity | null> {
  ...
  async findOneDynamicPurchasingDetail(
    conf?:
      | Pick<ParamsEntity, 'columnKey' | 'columnValue' | 'options'>
      | undefined
  ): Promise<PurchasingDetailEntity[] | []> {
    const res = await this.purchasingDetailDataSource.selectOneDynamic(conf!)
    return res
  }

  async checkQuantityTerminKontrakPks(
    type?: string | undefined,
    data?: TypePengajuanKontrakPks | undefined
  ): Promise<any> {
    const res = await this.purchasingDetailDataSource.availableQuantity(
      type!,
      data!
    )
    return res
  ...
  async updateTerminKontrakPks(
    ...
    await this.historyLogDataSource.insert(
      logUpdatePurchasingDetail.get('data')
    )

    return update
  }

  async deleteTerminKontrakPks(
    id?: number | undefined,
    user_id?: number | undefined
  ): Promise<any> {
    ...
    await this.historyLogDataSource.insert(
      logDeletePurchasingDetail.get('data')
    )
  ...
  async reportsPks(
    ...
    if (conf!.stockpile_name!)
      conf!.stockpile_name! = `'${conf!.stockpile_name!.join("','")}'`

    if (conf!.vendor_name!)
      conf!.vendor_name! = `'${conf!.vendor_name!.join("','")}'`

    const count = await this.pksCurahDataSource.summaryPks(isOa!, {
      period_from: conf?.period_from,
      period_to: conf?.period_from,
      stockpile_name: conf?.stockpile_name,
